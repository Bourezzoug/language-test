...
<div class="max-w-xl mx-auto">
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="bg-slate-900 px-6 py-5 text-white flex items-center justify-between">
            <h2 class="text-xl font-bold">Edit Test Section</h2>
            <a href="{{ route('admin.tests.sections.index', $section->test_id) }}" class="text-xs font-semibold text-slate-400 hover:text-white">
                Cancel
            </a>
        </div>

        <!-- Form -->
        <form action="{{ route('admin.sections.update', $section) }}" method="POST" class="p-6 space-y-5">
            @csrf
            @method('PUT')

            <!-- Test Title Display -->
            <div class="p-3 bg-slate-50 border border-slate-100 rounded-xl text-xs text-slate-500 font-medium">
                Test: <span class="text-slate-800 font-bold">{{ $section->test->title }}</span>
            </div>
            ...
            <div>
                ...
                <div class="mt-1">
                    <select name="type" id="type" required
                        class="block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-800 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm">
                        <option value="listening" {{ old('type', $section->type) === 'listening' ? 'selected' : '' }}>Listening Section</option>
                        <option value="reading" {{ old('type', $section->type) === 'reading' ? 'selected' : '' }}>Reading Section</option>
                        <option value="language_use" {{ old('type', $section->type) === 'language_use' ? 'selected' : '' }}>Language Use Section</option>
                    </select>
                </div>
                @error('type')
                    <p class="mt-1 text-xs text-rose-500 font-medium">{{ $message }}</p>
            ...
            <div>
                ...
                <div class="mt-1">
                    <input type="number" name="duration_minutes" id="duration_minutes" required min="1" value="{{ old('duration_minutes', $section->duration_minutes) }}"
                        class="block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-800 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm">
                </div>
            ...
            <div>
                ...
                <div class="mt-1">
                    <textarea name="instructions" id="instructions" rows="4"
                        class="block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-800 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm"
                        placeholder="Instructions displayed on top of student page when taking this section...">{{ old('instructions', $section->instructions) }}</textarea>
                </div>
                @error('instructions')
                    <p class="mt-1 text-xs text-rose-500 font-medium">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <div class="mt-1">
                    <input type="number" name="order" id="order" required min="0" value="{{ old('order', $section->order) }}"
                        class="block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-800 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm">
                </div>
                @error('order')
                    <p class="mt-1 text-xs text-rose-500 font-medium">{{ $message }}</p>
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-2">
                <a href="{{ route('admin.tests.sections.index', $section->test_id) }}" class="text-sm font-semibold text-slate-500 hover:text-slate-700 px-4 py-2">
                    Cancel
                </a>
