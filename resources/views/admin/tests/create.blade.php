...
<div class="max-w-xl mx-auto">
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="bg-slate-900 px-6 py-5 text-white flex items-center justify-between">
            <h2 class="text-xl font-bold">Create Placement Test</h2>
        ...
        <!-- Form -->
        <form action="{{ route('admin.tests.store') }}" method="POST" class="p-6 space-y-5">
            @csrf
            ...
            <div>
                ...
                <div class="mt-1">
                    <textarea name="description" id="description" rows="4"
                        class="block w-full rounded-lg border border-slate-300 px-3 py-2 text-slate-800 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm"
                        placeholder="Evaluation description and details...">{{ old('description') }}</textarea>
                </div>
                @error('description')
                    <p class="mt-1 text-xs text-rose-500 font-medium">{{ $message }}</p>
                @enderror
            ...
            <div class="space-y-3 pt-2">
                ...
                <label class="flex items-start gap-3 cursor-pointer select-none">
                    <input type="checkbox" name="is_active" value="1" checked class="mt-1 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                    <div>
                ...
                <label class="flex items-start gap-3 cursor-pointer select-none">
                    <input type="checkbox" name="show_result_to_student" value="1" checked class="mt-1 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                    <div>
